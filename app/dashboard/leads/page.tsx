        ...
        <button
          ...
          className={`px-6 py-2 rounded-lg text-lg font-semibold transition-all duration-300 ${
            leadType === "serviceLead"
              ...
              : "bg-gray-300 text-black hover:bg-purple-500 hover:text-white"
          }`}
        >
          Service Lead
        </button>
          ...
          <table className="w-full border-collapse border border-gray-300">
              ...
              <tr className="bg-gray-100">
                {leadType === "serviceLead" ? (
                  <>
                    ...
                    <th className="border border-gray-300 px-4 py-2">Interested In</th>
                  </>
                ) : (
                  <>
            ...
            <tbody>
              {filteredLeads.length > 0 ? (
                  ...
                  <tr key={lead.id} className="hover:bg-gray-200">
                    {leadType === "serviceLead" ? (
                      <>
                        ...
                        <td className="border border-gray-300 px-4 py-2">{lead.ownerName}</td>
                        <td className="border border-gray-300 px-4 py-2">{lead.emailId}</td>
                        <td className="border border-gray-300 px-4 py-2">{lead.interestedIn}</td>
                      </>
                    ) : (
                      <>
                        <td className="border border-gray-300 px-4 py-2">{lead.fullname || lead.name}</td>
                        <td className="border border-gray-300 px-4 py-2">{lead.email}</td>
                        <td className="border border-gray-300 px-4 py-2">{lead.role || "N/A"}</td>
                      </>
                    )}
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={leadType === "serviceLead" ? 4 : 3} className="text-center py-4 text-gray-500">
                    No leads found.
                  </td>
                </tr>
              )}
